...
const populatePostgres = (categories) => {
  let allCategoriesData = categories.map((category) =>
    reaktor.fetchProducts(category)
  )
  return Promise.all(allCategoriesData).then((productList) =>
    insertProductsIntoPostgres(productList)
  )
}
